  ...
  const fetchBuyers = async () => {
    try {
      ...
      const res = await axios.get(
        `${BASE_URL}/server/b2b_backend_function/getBuyers`
      );
      setBuyers(res.data);
    } catch (err) {
  ...
  const handleSearch = async (filterName, value) => {
    ...
    if (value !== "" && filterName === "word") {
      try {
        setLoading(true);
        const res = await axios.get(
          `${BASE_URL}/server/b2b_backend_function/searchBuyers/${value}`
        );
        setBuyers(res.data);
      } catch (err) {
        console.error("Failed to fetch order", err);
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Discover trusted buyers</p>
          </div>
          ...
          <div className="bg-card rounded-lg border p-6 space-y-4">
            ...
            <div className="flex gap-4">
              <div className="flex-1 relative">
                ...
                <Input
                  placeholder="Search by buyer name, address, or website..."
                  value={searchQuery}
